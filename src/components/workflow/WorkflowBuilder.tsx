...
// ── 5a. Range Target Imports ───────────────────────────────────────────
import { RangeTargetNode } from './RangeTargetNode';
import type {
  CanvasDragPayload,
  RangeTargetDragPayload,
  RangeTargetData,
} from '@/types/opforRangeTarget';
import { createRangeTargetFromTemplate } from '@/data/rangeTargets';
...
// ── 5a. Node types registration ───────────────────────────────────────
const nodeTypes = {
  opforNode:      OpforNode,
  phaseGroup:     PhaseGroupNode,
  arrowConnector: ArrowConnectorNode,
  rangeTargetNode: RangeTargetNode, // Added
};
...
function WorkflowBuilderInner({ campaign }: { campaign?: CampaignConfig | null }) {
  ...
  const availableVariables = useMemo((): Record<string, CanvasVariable> => {
    ...
    nodes.forEach(node => {
      ...
      robotConfig.variables?.forEach(varDef => {
        ...
        const instanceVarName = instance ? `${varDef.name}${instance.variablePrefix}` : varDef.name;
        vars[instanceVarName] = {
          ...
          category: varDef.name.split('_')[0],
        };
      });

      data.definition.outputs?.forEach(output => {
        const outputVarName = output.id.toUpperCase().replace(/-/g, '_');
        const instanceVarName = instance ? `${outputVarName}${instance.variablePrefix}` : outputVarName;
        vars[instanceVarName] = {
          name: instanceVarName,
          ...
          sourceNodeName: data.definition.name,
          variableType: 'output',
          category: outputVarName.split('_')[0],
        };
      });
    });
    return vars;
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (nodes.length > 0) {
        ...
        if (campaign) {
          const tacticsCovered = [...new Set(nodes.map(n => (n.data as OpforNodeData).definition?.tactic).filter(Boolean) as string[])];
          const required = campaign.jqrProfile?.requiredTactics ?? [];
          const covered = required.filter(t => tacticsCovered.includes(t)).length;
          const jqrProgress = required.length > 0 ? Math.round((covered / required.length) * 100) : 0;
          saveCampaignToIndex({
            ...campaign,
            canvasNodes: nodes,
            canvasEdges: edges,
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
        ...
        if (selected.length > 0) {
          selected.forEach(n => {
            if (n.type === 'opforNode') {
              ...
              addLogEntry('update', `Deleted TTP: ${data.definition.name}`, undefined, n.id);
            } else if (n.type === 'rangeTargetNode') {
              const data = n.data as RangeTargetData;
              addLogEntry('update', `Deleted Target: ${data.name}`, undefined, n.id);
            }
          });
      ...
      setEdges(eds => {
        ...
        if (selected.length > 0) addLogEntry('update', `Deleted ${selected.length} connection(s)`);
  ...
  const handleAddNodeFromAI = useCallback(
    (nodeDef: OpforNodeDefinition) => {
      ...
      addLogEntry('update', `Added from AI: ${safeNodeDef.name}`, safeNodeDef.description, newNode.id);
  ...
  const handleToggleCollapse = useCallback((groupId: string) => {
    ...
    setNodes(nds => nds.map(n => {
      ...
      if (childIds.has(n.id)) return { ...n, hidden: willCollapse };
      return n;
    }));
  ...
  const handleContainerizeSelected = useCallback((selectedIds: string[]) => {
    ...
    const contained = nodes.filter(n => selectedIds.includes(n.id) && n.type === 'opforNode' && !n.hidden && !n.parentId);
    ...
    addLogEntry('update', `Operator frame: ${contained.length} nodes containerized`);
  ...
  const handleSelectVariation = useCallback(
    (variation: GuidedVariation) => {
      ...
      const existingGroups = nodes.filter(n => n.type === 'phaseGroup');
      ...
      const orphanTails = nodes.filter(n => n.type === 'opforNode' && !nodesWithOutgoing.has(n.id) && !n.hidden);
      ...
      if (tailGroup) {
        ...
        const tgH = tgData.collapsed ? PILL_H : tgData.expandedHeight + GROUP_HEADER_H + GROUP_PADDING;
      ...
      const withTriggerHandles = (def: OpforNodeDefinition): OpforNodeDefinition => {
        ...
        return {
          ...def,
          inputs: [TRIGGER_IN, ...(def.inputs || []).filter(p => p.id !== 'trigger-in')],
          outputs: [TRIGGER_OUT, ...(def.outputs || []).filter(p => p.id !== 'trigger-out')],
        };
      };
      ...
      const childNodes: Node[] = variation.steps.map((step, index) => {
        ...
        const definition: OpforNodeDefinition = resolved
          ? { ...withTriggerHandles(ensureSafeNodeDef(resolved)), mitre: { tacticId: step.tactic, techniqueId: step.ttpId } }
          : {
              id: step.moduleKey, _key: step.moduleKey, name: step.displayName, icon: step.icon,
              tactic: (step.tactic as MitreTactic) || 'control', category: variation.phase, subcategory: '',
              description: `${step.ttpId} — ${step.ttpName}`, riskLevel: 'medium', estimatedDuration: 30,
              executionType: 'cobalt_strike', cobaltStrikeCommand: null, robotKeyword: null,
              robotTemplate: null, robotLibrary: null, shellCommand: null,
              ...
              parameters: [], requirements: { c2Server: false, listeners: [], payloads: [], sshConnections: [], externalTools: [], libraries: [] },
              metadata: { version: '1.0', lastUpdated: '', updatedBy: '', validationStatus: 'draft', changeLog: '', owner: '', status: 'active', tags: [] },
              outputObjects: [], mitre: { tacticId: step.tactic, techniqueId: step.ttpId },
            };

        const initialParams: Record<string, string | number> = {};
        definition.parameters?.forEach(param => { if (param.default !== undefined) initialParams[param.id] = param.default; });
        ...
        return {
          ...
          parentId: groupId,
          extent: 'parent' as const,
          position: { x: GROUP_PADDING + index * (CHILD_NODE_W + CHILD_GAP_X), y: GROUP_HEADER_H + GROUP_PADDING },
          data: { definition, parameters: initialParams, label: step.displayName, validationState: 'unconfigured' } as OpforNodeData,
        };
      });
      ...
      childIds.forEach((id, i) => {
        ...
        newEdges.push({
          ...
          source: childIds[i - 1], target: id,
          sourceHandle: 'trigger-out', targetHandle: 'trigger-in',
          animated: true, style: { stroke: 'hsl(210, 100%, 50%)', strokeWidth: 2 },
        });
      });
      ...
      if (prevTailOpfor) {
        newEdges.push({
          ...
          source: prevTailOpfor.id, target: childIds[0],
          sourceHandle: 'trigger-out', targetHandle: 'trigger-in',
          animated: false, hidden: true, type: 'smoothstep',
      ...
      if (tailGroup) {
        ...
        extraNodes.push({
          id: arrowId, type: 'arrowConnector', draggable: false, selectable: false,
          position: { x: groupX + PILL_W / 2 - 10, y: tailGroup.position.y + tgH + LANE_GAP_Y / 2 - 10 },
          data: {},
        });
      }
      ...
      addLogEntry('update', `Guided: placed "${variation.name}"`);
      toast({ title: '✓ Variation placed' });
  ...
  const onConnect = useCallback(
    (connection: Connection) => {
      setEdges(eds => addEdge({ ...connection, animated: true, style: { stroke: 'hsl(210, 100%, 50%)', strokeWidth: 2 } }, eds));
  ...
  const onDrop = useCallback(
    async (event: React.DragEvent) => {
      ...
      let parsed: CanvasDragPayload;
      try {
        parsed = JSON.parse(raw) as CanvasDragPayload;
      } catch {
        return;
      }

      const dragType = (parsed as any).__dragType ?? 'opforNode';
      const payload = (parsed as any).payload ?? parsed;
      const position = screenToFlowPosition({ x: event.clientX, y: event.clientY });

      // Branch 1: Range Target Drop
      if (dragType === 'rangeTarget') {
        const rt = payload as RangeTargetDragPayload;
        const existingNames = new Set(
          nodes
            .filter((n) => n.type === 'rangeTargetNode')
            .map((n) => (n.data as RangeTargetData).name)
        );
        const targetData = createRangeTargetFromTemplate(rt.kind, existingNames);
        const newNode: Node = {
          id: targetData.targetId,
          type: 'rangeTargetNode',
          position,
          data: targetData,
        };
        setNodes((nds) => nds.concat(newNode));
        addLogEntry('update', `Added Range Target: ${targetData.name}`, targetData.kind);
        return;
      }

      // Branch 2: Standard Opfor Node Drop
      let nodeDef = payload as OpforNodeDefinition;
      const moduleKey = nodeDef._key || nodeDef.id;
      try {
        const modulePayload = await libraryModuleService.getModulePayload(moduleKey);
        if (modulePayload) {
          nodeDef = { ...nodeDef, ...modulePayload, _key: nodeDef._key, id: nodeDef.id };
        }
      } catch (e) {
        console.warn(`Could not fetch payload for ${moduleKey}`, e);
      }
    ...
    [screenToFlowPosition, nodes, setNodes, addLogEntry, ensureSafeNodeDef, fitView]
  );
  ...
  const handleValidate = useCallback(() => {
    ...
    const updatedNodes = nodes.map(node => {
      if (node.type !== 'opforNode') return node;
      ...
      const validationState: OpforNodeData['validationState'] = nodeIssues.length > 0 ? 'configured' : 'validated';
    ...
    if (hasIssues) {
      ...
      toast({ title: '⚠️ Validation Failed', description: `Found ${issues.length} issue(s).`, variant: 'destructive' });
      return;
    }
    addLogEntry('validation', 'TTP chain validated', `${nodes.length} techniques validated`);
    toast({ title: '✅ Chain Valid' });
  ...
  const handleExport = useCallback(() => setShowReadiness(true), []);

  const handleConfirmExport = useCallback(() => {
    setShowReadiness(false);
    addLogEntry('export', 'Operation card exported', globalSettings.executionPlanName);
    toast({ title: '📄 Card Exported' });
  }, [globalSettings.executionPlanName, toast, addLogEntry]);

  const handleReset = useCallback(() => {
    setNodes([]); setEdges([]); setSelectedNode(null);
    setExecutionState(initialExecutionState); hasInitialNode.current = false;
    setInfrastructureStatus({ c2Connected: false, robotAvailable: false, listeners: [], payloads: [] });
  ...
  const handleLoadWorkflow = useCallback(
    (workflow: WorkflowFile) => {
      setNodes(workflow.nodes); setEdges(workflow.edges); setGlobalSettings(workflow.globalSettings);
      hasInitialNode.current = true;
      setTimeout(() => fitView({ padding: 0.2, minZoom: workflow.viewport.zoom, maxZoom: workflow.viewport.zoom }), 100);
  ...
  const panelNodes = useMemo(() => 
    nodes.filter(n => n.type === 'opforNode').map(n => ({ id: n.id, data: n.data as OpforNodeData })), 
  [nodes]);

  const panelSelectedNode = useMemo(() => {
    if (!selectedNode) return null;
    if (selectedNode.type === 'opforNode' || selectedNode.type === 'rangeTargetNode') {
      return {
        id: selectedNode.id,
  ...
  const taggedGroupCount = nodes.filter(n => 
    n.type === 'phaseGroup' && (n.data as any).source === 'operator' && (n.data as any).contributionStatus === 'ready'
  ).length;

  return (
    <div className="h-screen w-screen flex flex-col bg-background overflow-hidden">
      <OperatorHeader infrastructureStatus={infrastructureStatus} globalSettings={globalSettings} />
      <Toolbar
        viewMode={viewMode} lifecycleStage={lifecycleStage} onViewModeChange={setViewMode}
        onLifecycleChange={setLifecycleStage} onValidate={handleValidate} onSimulate={handleSimulate}
        onExport={handleExport} onReset={handleReset} onSave={() => setSaveDialogOpen(true)}
        onLoad={() => setLoadDialogOpen(true)} nodes={nodes} jqrProfile={jqrProfile}
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        <div className="w-72 border-r border-panel-border flex-shrink-0">
          <NodePalette
            onDragStart={onDragStart} onSelectVariation={handleSelectVariation}
            tacticFilter={tacticFilter} onClearTacticFilter={() => setTacticFilter(null)}
            framingMode={framingMode} onToggleFramingMode={() => setFramingMode(f => !f)}
            selectedNodeIds={framingMode ? nodes.filter(n => n.selected && n.type === 'opforNode' && !n.parentId).map(n => n.id) : []}
        ...
        {viewMode === 'script' ? (
          <div className="flex-1 flex min-w-0">
            ...
            <div className="flex flex-col flex-shrink-0" style={{ width: 'auto' }}>
              <CampaignStatusBar
                lifecycleStage={lifecycleStage} onLifecycleChange={setLifecycleStage}
                nodes={nodes} jqrProfile={jqrProfile} onFilterTactic={setTacticFilter}
              ...
              <CollapsiblePropertiesPanel
                selectedNode={panelSelectedNode} nodes={panelNodes} timerLog={executionLog}
                globalSettings={globalSettings} onSettingsChange={setGlobalSettings}
                onNodeDataChange={handleNodeDataChange} onAddNodeToCanvas={handleAddNodeFromAI}
                availableVariables={availableVariables} edges={edges} nodeInstances={nodeInstances}
                connectionContext={connectionContext} nodeMap={nodeMap} infrastructureStatus={infrastructureStatus}
              />
            </div>
          </div>
        ) : (
          <>
            <div className="flex-1 min-w-0 relative" ref={reactFlowWrapper}>
              <ReactFlow
                ...
                onDrop={onDrop} onDragOver={onDragOver} isValidConnection={() => true}
                nodeTypes={nodeTypes} defaultViewport={{ x: 0, y: 0, zoom: 0.7 }}
                minZoom={0.3} maxZoom={1.5} snapToGrid snapGrid={[15, 15]}
                proOptions={{ hideAttribution: true }} className="bg-canvas"
                deleteKeyCode="Delete" multiSelectionKeyCode="Shift" selectionOnDrag={false}
                panOnDrag={!framingMode}
              >
                <Background variant={BackgroundVariant.Dots} gap={20} size={1} color="hsl(222, 15%, 18%)" />
                <Controls className="!bg-panel !border-panel-border !shadow-lg" showZoom={false} showFitView={false} />
                <MiniMap
                  nodeColor={node => {
                    const data = node.data as OpforNodeData;
                  ...
                  maskColor="hsla(222, 20%, 8%, 0.8)" className="!bg-panel !border-panel-border"
                />
                {nodes.length === 0 && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="text-center space-y-4 max-w-md px-8">
                      ...
                      <h2 className="text-2xl font-mono font-medium uppercase tracking-widest text-white">Awaiting Instructions</h2>
                      <p className="text-zinc-500 text-sm font-mono uppercase tracking-tight">Use the Tactic Library to create your attack chain</p>
                    </div>
                  </div>
                )}
              </ReactFlow>
            ...
            <div className="flex flex-col flex-shrink-0" style={{ width: 'auto' }}>
              <CampaignStatusBar
                ...
                nodes={nodes} jqrProfile={jqrProfile} onFilterTactic={setTacticFilter}
                taggedGroupCount={taggedGroupCount}
                onPublish={() => toast({ title: '🚀 Published' })}
              />
              <CollapsiblePropertiesPanel
                selectedNode={panelSelectedNode} nodes={panelNodes} timerLog={executionLog}
                globalSettings={globalSettings} onSettingsChange={setGlobalSettings}
      ...
      <ExecutionPanel
        ref={executionPanelRef} isOpen={executionPanelOpen} onToggle={() => setExecutionPanelOpen(!executionPanelOpen)}
        onClose={() => setExecutionPanelOpen(false)} executionState={executionState} onRun={handleRunExecution}
        onStop={handleStopExecution} onRerun={handleRerunExecution} onHighlightNode={handleHighlightNode}
        scriptContent={generatedScript?.full ?? ''} apiBaseUrl="http://localhost:8001"
      />

      <SaveLoadDialog open={saveDialogOpen} onClose={() => setSaveDialogOpen(false)} mode="save" currentWorkflow={{ nodes, edges, globalSettings, viewport: getViewport() }} />
      <SaveLoadDialog open={loadDialogOpen} onClose={() => setLoadDialogOpen(false)} mode="load" onLoad={handleLoadWorkflow} />
      {showReadiness && <ReadinessCheck nodes={nodes} edges={edges} jqrProfile={jqrProfile} onConfirm={handleConfirmExport} onClose={() => setShowReadiness(false)} />}
    </div>
  );
}
