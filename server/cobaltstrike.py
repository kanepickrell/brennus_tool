...
def stop_c2() -> Dict[str, Any]:
    ...
    if not _connected:
        _log("⚠ Not connected to any teamserver")
...
class Session:
    """Represents a beacon session with attribute access"""
    ...
    def __repr__(self):
        return f"Session(beacon_id={self.beacon_id}, ip={self.ip}, user={self.username})"
    
    def __str__(self):
        return f"<Session {self.beacon_id} @ {self.ip} ({self.username})>"
    
    def __getitem__(self, key):
        """Allow dictionary-style access for Robot Framework compatibility"""
...
def get_sessions_by_ip(ip: str) -> List[Session]:
    ...
    # Find existing sessions for this IP
    matching_sessions = [s for s in _sessions.values() if s.ip == ip]
    
    # If no sessions exist, create a mock one (simulates beacon callback)
    if not matching_sessions:
        _log(f"  No existing sessions found, simulating new beacon callback...")
...
def get_sessions_by_user(username: str) -> List[Session]:
    ...
    Returns:
        List of Session objects matching the username
    """
...
def run_getuid(session: Session) -> str:
    """
    ...
    Args:
        session: Session object to run command on
        
    Returns:
        User identity string (e.g., "DOMAIN\\username")
    """
...
def issue_shell_cmd(session: Session, command: str) -> Dict[str, Any]:
    """
    ...
    _log(f"  Command: {command[:50]}{'...' if len(command) > 50 else ''}")
    time.sleep(0.3)
    ...
    if command.startswith("whoami"):
        ...
        output = f" Volume in drive C has no label.\n Directory of C:\\Users\\{session.username}\n\n01/27/2026  10:30 AM    <DIR>          .\n01/27/2026  10:30 AM    <DIR>          ..\n01/27/2026  10:30 AM    <DIR>          Desktop\n01/27/2026  10:30 AM    <DIR>          Documents\n"
    elif command.startswith("tasklist"):
        output = f"Image Name                     PID Session Name        Mem Usage\n========================= ======== ================ ============\nSystem Idle Process              0 Services                   8 K\nSystem                           4 Services                 180 K\nexplorer.exe                  {session.pid} Console                  {random.randint(50000, 150000)}K\n"
    elif command.startswith("reg "):
        output = f"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\n    Update    REG_SZ    C:\\Users\\{session.username}\\AppData\\Local\\Temp\\update.exe\n"
    elif command.startswith("arp"):
        output = f"Interface: {session.ip} --- 0x5\n  Internet Address      Physical Address      Type\n  192.168.1.1           00-11-22-33-44-55     dynamic\n  192.168.1.254         66-77-88-99-aa-bb     dynamic\n"
        ...
        if "/create" in command:
            output = 'SUCCESS: The scheduled task "Updates" has successfully been created.'
...
def issue_powershell_cmd(session: Session, command: str) -> Dict[str, Any]:
    """
    ...
    _log(f"Executing PowerShell command on beacon {session.beacon_id}...")
    _log(f"  Command: {command[:50]}{'...' if len(command) > 50 else ''}")
    time.sleep(0.3)
    ...
    if "Compress-Archive" in command:
        ...
        output = f"Handles  NPM(K)    PM(K)      WS(K)     CPU(s)     Id  SI ProcessName\n-------  ------    -----      -----     ------     --  -- -----------\n    523      25    78432      89244       2.34   {session.pid}   1 explorer"
    else:
        output = f"PowerShell command executed: {command}"
    
    _log(f"  ✓ PowerShell command completed")
...
def run_mimikatz(session: Session) -> Dict[str, Any]:
    """
    ...
    rand_auth_id = random.randint(100000, 999999)
    rand_logon_id = random.randint(0x10000, 0xfffff)
...
Authentication Id : 0 ; {rand_auth_id} (00000000:{rand_logon_id:08x})
Session           : Interactive from 1
User Name         : {session.username}
Domain            : {session.domain}
Logon Server      : {session.computer}
Logon Time        : 1/27/2026 10:30:00 AM
...
"""
    ...
    return {
        ...
        "credentials": creds,
        "beacon_id": session.beacon_id,
        "result": session  # Pass through session for chaining
    }


def run_bjump(session: Session, method: str, target_ip: str, listener: str) -> Dict[str, Any]:
    """
    Lateral movement using various techniques (psexec64, winrm64, etc.)
...
def Lateral_Move_Psexec(session: Session, target_ip: str, listener: str) -> Dict[str, Any]:
    """
    Lateral movement using PsExec64
    
    Robot Keyword: Lateral_Move_Psexec
    
    This is a convenience wrapper around run_bjump specifically for PsExec.
    """
    if isinstance(session, dict):
        session = Session(session)
...
def run_bspawnas(session: Session, domain: str, username: str, password: str, listener: str) -> Dict[str, Any]:
    """
...
def run_binject(session: Session, pid: int, listener: str, arch: str = "x64") -> Dict[str, Any]:
    """
    ...
    # Create new session (same user, different process)
    new_session = _create_mock_session(
        ip=session.ip,
        username=session.username,
        domain=session.domain,
    ...
    new_session.pid = pid
    new_session.process = "explorer.exe"
...
def run_bportscan(session: Session, targets: str, ports: str, method: str = "icmp") -> Dict[str, Any]:
    """
    ...
    _log(f"  ✓ Scan completed - found {len(results)} open port(s)")
...
def upload_file(session: Session, local_file: str, remote_path: str) -> Dict[str, Any]:
    """
...
def download_file(session: Session, remote_file: str) -> Dict[str, Any]:
    """
...
def session_sleep(session: Session, sleep_time: int, jitter: int = 0) -> Dict[str, Any]:
    """
...
def kill_session(session: Session) -> Dict[str, Any]:
    """
    ...
    if beacon_id in _sessions:
        del _sessions[beacon_id]
    if beacon_id in _beacons:
        del _beacons[beacon_id]
...
def run_btimestomp(session: Session, target_file: str, reference_file: str) -> Dict[str, Any]:
    """
...
def Initial_Access(target_ip: str, username: str, password: str, beacon_path: str) -> Dict[str, Any]:
    ...
    Note: Uses underscore in name to avoid Robot Framework keyword conflict.
    """
    ...
    # Create the session (beacon callback)
